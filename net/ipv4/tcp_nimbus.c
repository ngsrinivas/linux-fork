                                  ...
                                  u32 min_rtt)
{
  ...
  do_div(zt, routt);
  zt -= rint;
  pr_info("Estimated cross traffic: %lld bps\n", zt);
                               ...
                               u32 zt)
{
  ...
  delay_term = est_bandwidth * delay_diff;
  do_div(delay_term, last_rtt);
  delay_term = delay_term * NIMBUS_BETA;
  do_div(delay_term, NIMBUS_FRAC_DR);
  ...
  single_seg_rate = single_seg_bps(last_rtt);
  if (new_rate < single_seg_rate) new_rate = single_seg_rate;
  if (new_rate > LINK_CAP) new_rate = NIMBUS_MAX_UTIL * LINK_CAP / NIMBUS_FRAC_DR;
...
void tcp_nimbus_cong_control(struct sock *sk, const struct rate_sample *rs)
{
  ...
  struct nimbus *ca = inet_csk_ca(sk);
  if (rate_sample_valid(rs) && epoch_elapsed(ca)) {
